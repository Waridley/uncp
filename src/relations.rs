...
impl RelationStore {
    ...
    pub fn insert<K: RelationKey>(&mut self, data: DataFrame) -> DetectorResult<()> {
        ...
        // Update metadata
        let mut metadata = self.metadata.get(&type_id)
            .cloned()
...
mod tests {
    ...
    fn test_multiple_relation_types() {
        ...
        let filename_paths_series = Series::new(
            "file_paths",
            vec![Series::new("", vec!["/home/user/document.pdf", "/backup/document.pdf"])]
        );
