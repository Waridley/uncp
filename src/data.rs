...
impl ScanState {
    ...
    fn files_to_dataframe(&self, files: Vec<FileRecord>) -> PolarsResult<DataFrame> {
        ...
        let modified: Vec<i64> = files.iter().map(|f| f.modified.timestamp_nanos_opt().unwrap_or(0)).collect();
    ...
    pub fn files_needing_processing(&self, system_name: &str) -> DetectorResult<DataFrame> {
        let filter_expr = match system_name {
            ...
            _ => return Err(DetectorError::Config(format!("Unknown system: {}", system_name))),
        };
    ...
    pub fn update_processing_flags(&mut self, paths: &[String], flag: &str, value: bool) -> DetectorResult<()> {
