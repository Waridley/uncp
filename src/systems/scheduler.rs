...
impl SystemScheduler {
    ...
    pub async fn run_all(&self, state: &mut ScanState, memory_mgr: &mut MemoryManager) -> SystemResult<()> {
        // Filter systems that can run
        let mut runnable: Vec<&Box<dyn SystemRunner>> = self
            .systems
            .iter()
            .filter(|s| s.can_run(state))
            .collect();
        ...
        for system in runnable.into_iter() {
            debug!("Scheduler: running system {} (priority {})", system.name(), system.priority());
