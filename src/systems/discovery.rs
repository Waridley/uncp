...
use crate::systems::{System, SystemRunner, SystemContext, SystemProgress, yield_periodically};
...
impl FileDiscoverySystem {
    ...
    pub async fn discover_files(&self, context: &SystemContext) -> SystemResult<Vec<FileRecord>> {
        info!("Discovery: starting scan of {} paths", self.scan_paths.len());
        ...
        for scan_path in &self.scan_paths {
            ...
            let files = self.discover_files_in_path(scan_path, context, &mut progress, &mut last_yield).await?;
    ...
    fn determine_file_type(&self, path: &Path) -> FileKind {
        ...
        match mime_str {
            ...
            "application/zip" | "application/x-tar" | "application/gzip" | "application/x-rar-compressed" => FileKind::Archive,
            _ => {
                // Fallback to extension-based detection
                if let Some(ext) = path.extension().and_then(|e| e.to_str()) {
                    match ext.to_lowercase().as_str() {
                        "txt" | "md" | "rs" | "py" | "js" | "html" | "css" | "json" | "xml" | "yaml" | "toml" => FileKind::Text,
                        "jpg" | "jpeg" | "png" | "gif" | "bmp" | "tiff" | "webp" | "svg" => FileKind::Image,
...
impl SystemRunner for FileDiscoverySystem {
    async fn run(&self, state: &mut ScanState, _memory_mgr: &mut MemoryManager) -> SystemResult<()> {
        ...
        state.add_files(files).map_err(|e| SystemError::ExecutionFailed {
