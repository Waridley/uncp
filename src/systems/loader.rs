...
impl SystemRunner for ContentLoaderSystem {
	async fn run(&self, pool: std::sync::Arc<crate::pool::DataPool>) -> SystemResult<()> {
		loop {
			...
			let (mut items, size_by_key): (
				Vec<DirEntryId>,
				std::collections::HashMap<(usize, usize), u64>,
			) = {
				...
				for i in 0..state.data.height() {
					if let (Some(idx), Some(r#gen), Some(sz)) =
						(idx_ca.get(i), gen_ca.get(i), sizes.get(i))
					{
			...
			for id in items.into_iter().take(self.max_per_run) {
				let _expected = size_by_key.get(&id.raw_parts()).copied().unwrap_or(0) as usize;
				// Read file bytes without holding the memory lock
				let bytes = match smol::fs::read(id.resolve()).await {
					Ok(b) => b,
					Err(e) => {
						warn!(?e, "ContentLoader: failed to read content");
						continue;
					}
				};
				...
				let load_ok = match mem.put_file(id.resolve(), crate::memory::FileBytes::new(bytes))
				{
					Ok(_) => true,
					Err(e) => {
						warn!(?e, "ContentLoader: cache put failed");
						false
					}
				};
				if load_ok {
